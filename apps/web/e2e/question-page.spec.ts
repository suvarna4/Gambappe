...
test.describe('reveal sequence (§10.3, WS7-T3)', () => {
  ...
  test('SW3-T2 obituary handoff: a real broken streak swaps the share button for ObituaryCard', async ({
    page,
  }) => {
    await page.emulateMedia({ reducedMotion: 'reduce' });
    // `question.question_date` is what `RevealSequence` actually reads for the obituary handoff
    // (its own `question` prop — the real server-rendered row, not the mocked reveal payload's
    // cosmetic `question_date` field below) — every other test in this file leaves it `null` on
    // purpose (see `seedQuestion`'s header comment) since they don't need it, but the obituary
    // hook does, so this one needs a real, collision-free date (golden-loop.spec.ts's own
    // far-future-year pattern, §6.6's `questions_daily_date_uq` is per-date-per-`kind='daily'`).
    const { question } = await seedQuestion(
      {},
      {
        status: 'revealed',
        ...
        yesLabel: 'Yes it will',
        revealedAt: new Date(),
        questionDate: `${2100 + Math.floor(Math.random() * 400)}-01-15`,
      },
    );
    ...
    await page.route(`**/api/v1/questions/${question.slug}/reveal`, (route) =>
      route.fulfill({
        ...
        json: revealMockBody(question, {
          ...
          edge: -0.71,
          percentile: 12,
          // current === 0 on a loss, previous (current - delta) = 4 >= OBITUARY_MIN_STREAK.
          streak: { current: 0, best: 4, delta: -4, freeze_used: false },
          badges: [],
        }),
      }),
    ...
    await expect(obituary).toContainText('Here lies a 4-day streak.');
    await expect(obituary).toContainText('RIP 4');
    // pick.side = 'no' (the real seeded question's `no_label`, factory default "No" — the
    // obituary reads the REAL server-rendered `question` prop's labels, never the mocked reveal
    // payload's own cosmetic `no_label`), yes_price_at_entry 0.71 -> no-side implied 29¢.
    await expect(obituary).toContainText('Died holding No @ 29¢.');
    // The obituary REPLACES the plain share button (§2.6 "swaps the share affordance") — same
    // reveal, one or the other, never both.
    await expect(page.getByTestId('share-receipt-button')).not.toBeVisible();

    // "Share the obituary" opens the same real ShareSheet the plain button would have.
