...
5. Transaction: `SELECT status FROM questions WHERE id = ? FOR SHARE` and re-check `status='open'` (the row share-lock serializes against the lock job's `FOR UPDATE`, so a pick can never slip between the lock job's status flip and its counter snapshot); insert pick; increment `questions.yes_count|no_count`. Unique-violation → return existing pick with `ALREADY_PICKED` (409, body includes the existing pick — idempotent-friendly).
...
**RevealPayload** (type in `core/types`): `{question, outcome, crowd:{yes,no,pct_yes}, viewer?: {pick, result, edge, percentile: number|null, streak:{current,best,delta,freeze_used}, badges:['called_it'?]}, narrative_line, share:{page_url, og_url, card_urls}}`. The `viewer` block is fetched client-side (§10.2 keeps SSR viewer-free). `percentile` is **nullable by contract** — at P0 (before WS3-T5 ships) it is always null and the UI omits the stat.
