...
- **Jul 23, 2026 — §6.3, fix a server-action-only crash on sign-in failure (WS25-T7).** A real user hit "Application error: a server-side exception has occurred" clicking Save, even after WS25-T6 shipped — a different, more severe failure mode than anything WS25 addressed before: a raw Next.js Server Action crash, not an Auth.js-rendered page. Root cause: `app/claim/actions.ts`'s `signInWithGoogle`/`signInWithTwitter`/`signInWithEmail` call `next-auth`'s server-action `signIn()` wrapper (`next-auth/src/lib/actions.ts`), which always invokes `@auth/core`'s `Auth()` with a `raw` marker. Inside `Auth()`, when the call is raw and the thrown error is an `AuthError` (e.g. WS25-T4's `EmailSignInError`, thrown on an ordinary §14.1 rate-limit trip), `Auth()` re-throws the raw error instead of computing its usual `pages.error`/`pages.signIn` redirect — `auth.ts`'s `pages` config (WS25-T6) is never consulted on this path. Uncaught, that propagates out of the Server Action as a framework crash. This gap survived every prior WS25 verification step (T4's live checks, T6's two independent review rounds and its own `auth-error-routing.test.ts`) because all of it POSTed directly to the `/api/auth/signin/email` route handler, which never sets `raw` — only the real `<form action={signInWithEmail}>` in `ClaimEntry`, via `next-auth`'s server-action `signIn`, does. Fix: new `apps/web/lib/auth-signin-redirect.ts` exports `redirectOnAuthError`, called from a `try`/`catch` (`signInOrRedirect`) wrapped around all three provider calls in `actions.ts`. It first calls Next.js's documented `unstable_rethrow` (the framework's own pattern for wrapping an API — here, `signIn()`'s internal success-path `redirect()` — that uses thrown errors for control flow, before doing custom error handling) so a normal successful sign-in is never swallowed; then, for an `AuthError`, redirects to `/claim?error=Configuration` — the same masked destination `Auth()`'s non-raw path already produces for `EmailSignInError` specifically, the one error type actually confirmed reachable here (§14.1's rate-limit trip); it isn't in `@auth/core`'s `clientErrors` allowlist, so `Configuration` matches what WS25-T6 already ships and tests. (One allowlisted type, `AccessDenied`, is technically reachable from the same initial-signin call via `@auth/core`'s `send-token.ts` if the `signIn` callback rejects — dormant today since `auth.ts`'s callback only ever rejects a `google` account, not `email`; see `auth-signin-redirect.ts`'s own comment for the follow-up note if that callback's gating changes.) `signInWithGoogle`/`signInWithTwitter` get the same guard as cheap, harmless defensive parity — their proximate crash trigger (OAuth callback errors) mostly surfaces at the callback step, a route-handler GET unaffected by `raw`, and no concrete reachable initial-step `AuthError` was confirmed in `@auth/core@0.41.2`'s source for this app's OAuth config; the guard costs nothing and removes the need to re-verify that absence every time the library or config changes. New `apps/web/test/auth-signin-redirect.test.ts` exercises `redirectOnAuthError` directly (importable under vitest without `../../auth`, same testability rationale as `auth-magic-link-send.ts`): confirms an `AuthError` redirects to `/claim?error=Configuration`, a captured real `next/navigation` redirect error is re-thrown unchanged (not misrouted to the error page), and an unrecognized error is left for the caller to re-throw. Live-verified against a real `next build && next start`, driving the actual `<form action={signInWithEmail}>` via Playwright (not curl) through 7 real Save submissions past the 5/hour rate limit: reverting this fix locally reproduced the exact "Application error" crash on the request that trips the limit; with the fix, that same request redirects to `/claim?error=Configuration` with the branded retry message, no crash.
